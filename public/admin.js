...
const CONFIG_GROUPS=[
  ...
  {id:'proxy-pool',title:'代理池',fields:[
    ...
    {path:'proxy_pool.urls',label:'代理 URL 列表',type:'list',help:'每行一个 http:// / https:// 代理地址，或单独一行 direct 表示服务器直连。Mihomo 需使用 mixed/http 入口，不支持 socks5://。',mono:true,rows:8,full:true},
    {path:'proxy_pool.cooldown_seconds',label:'冷却秒数',type:'number',help:'429 或网络错误后，该代理会进入冷却窗口。设为 0 表示不进入冷却，仅影响当前这一次切换。',min:0},
  ...
  {id:'security',title:'鉴权与安全',fields:[
    ...
    {path:'upstream_blocker.enabled',label:'上游关键词拦截转错误',type:'checkbox',help:'命中关键词时，不再向下游返回 200 成功响应，而是改为上游拦截错误。'},
    {path:'upstream_blocker.block_empty_response',label:'上游空回复转错误',type:'checkbox',help:'上游最终回复为空白时，不再向下游返回 200 成功响应，而是改为上游拦截错误。'},
